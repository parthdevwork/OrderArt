...
const CompletedOrders = (props) => {
	...
	const renderItem = ({ item }) => (
		<OrderCard order={item} onPress={() => props.navigation.navigate('OrderDetails', { id: item.id })} />
	);
	...
	const renderData = () => (
		<>
			...
			{pageData.isRefreshing && !props.orders.length && <Loader loading={pageData.isRefreshing} inPageLoader={true} />}
...
const mapStateToProps = state => ({
	...
	orders: state.orders.completedOrdersList,
	isLoaded: state.orders.isCompletedOrderListLoaded
});

const _CompletedOrdersScreen = connect(
	mapStateToProps,
	bindAction
)(CompletedOrders);

export default _CompletedOrdersScreen;